...
# Intended to run as root under systemd (theexonet-permissions.service).
set -euo pipefail
...
LOCK_FILE="${THEEXONET_PERMISSIONS_LOCK:-/run/theexonet-permissions.fix.lock}"
POLL_INTERVAL="${POLL_INTERVAL:-30}"
DEBOUNCE_SECONDS="${DEBOUNCE_SECONDS:-10}"
JOURNAL_UNITS="${THEEXONET_JOURNAL_UNITS:-theexonet-api theexonet-admin theexonet-moderator theexonet-docs theexonet-status}"
JOURNAL_PATTERN="${THEEXONET_JOURNAL_PATTERN:-Permission denied|Access to the path|UnauthorizedAccessException|EACCES|is denied}"

LAST_FIX_EPOCH=0

log_msg() {
  logger -t theexonet-permissions -- "$*"
  echo "[theexonet-permissions] $*"
}

require_root() {
  if [ "$(id -u)" -ne 0 ]; then
    echo "Must run as root (use theexonet-permissions.service)." >&2
    exit 1
  fi
}
...
run_fix() {
  ...
  if "${FIX_SCRIPT}" -q; then
    if "${AUDIT_SCRIPT}" -q; then
      ...
      log_msg "WARNING: issues remain after fix — check audit-theexonet-permissions"
    fi
  else
    log_msg "ERROR: fix-theexonet-permissions failed"
    return 1
  fi
}
