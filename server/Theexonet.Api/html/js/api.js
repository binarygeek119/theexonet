...
const TOKEN_KEY = "theexonet_token";
const MINE_KEY = "theexonet_mineId";
const USER_KEY = "theexonet_username";

export class TheexonetApi {
  constructor(baseUrl = "") {
  ...
  adminRegenerateAllOffworldNewsReporterPortraits() {
...
function formatHttpError(response, path = "") {
  ...
  if (response.status === 404) {
    ...
    return `That API route is not on the server yet${route}. Redeploy Theexonet.Api, restart the service, then hard-refresh this page.`;
  }

  if (response.status === 405) {
    return "API route not available. Requests must go to the API host (theexonetapi), not the game site. Hard-refresh the page or redeploy html/.";
  }
