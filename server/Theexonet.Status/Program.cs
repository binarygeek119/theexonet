...
builder.Services.AddHttpClient("TheexonetApi");
builder.Services.AddHttpClient("TheexonetPortal");
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var apiClient = httpClientFactory.CreateClient("TheexonetApi");
    ...
    var portalClient = httpClientFactory.CreateClient("TheexonetPortal");
    ...
    CancellationToken cancellationToken) =>
{
    ...
    PublicOpenAiStatusDetailResponse? theexonet = null;
    string? theexonetError = null;
    try
    {
        using var response = await apiClient.GetAsync($"{apiBaseUrl}/api/status/openai", cancellationToken);
        if (response.IsSuccessStatusCode)
        {
            theexonet = await response.Content.ReadFromJsonAsync<PublicOpenAiStatusDetailResponse>(cancellationToken);
        }
        else
        {
            theexonetError = $"HTTP {(int)response.StatusCode} {response.ReasonPhrase}";
        }
    }
    catch (Exception ex)
    {
        theexonetError = ex.Message;
    }
