...
function renderPage(data) {
  ...
  if (!theexonet) {
    ...
    els.apiError.textContent = data.theexonetError || "Could not load AI content status from theexonet API";
    return;
  }

  els.apiError.textContent = data.theexonetError || "—";
  els.apiKey.textContent = formatYesNo(theexonet.apiKeyConfigured);
  els.requestsTotal.textContent = formatCount(theexonet.totalRequests);
  els.requestsToday.textContent = formatCount(theexonet.requestsToday);
  ...
  const activeCount = (theexonet.gameFeatures ?? []).filter((feature) => feature.enabled).length;
  const totalFeatures = theexonet.gameFeatures?.length ?? 0;
