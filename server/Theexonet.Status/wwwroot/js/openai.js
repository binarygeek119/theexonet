...
const els = {
  ...
  theexonetApiKey: document.getElementById("theexonet-api-key"),
  ...
  theexonetApiError: document.getElementById("theexonet-api-error"),
  linkTheexonetOpenAiJson: document.getElementById("link-theexonet-openai-json"),
...
function renderTheexonet(theexonet, theexonetError, apiPublicUrl, gameUrl) {
  els.linkTheexonetOpenAiJson.href = `${apiPublicUrl.replace(/\/$/, "")}/api/status/openai`;
  els.linkGameExonet.href = gameUrl;

  if (!theexonet) {
    els.theexonetApiError.textContent = theexonetError || "Could not load theexonet OpenAI status from API";
    return;
  }

  els.theexonetApiError.textContent = theexonetError || "—";
  els.theexonetApiKey.textContent = formatYesNo(theexonet.apiKeyConfigured);
  ...
  renderUsageBars(theexonet.requestsByCategory, theexonet.totalRequests);
  renderFeatures(theexonet.gameFeatures);

  const cfg = theexonet.configuration;
  if (cfg) {
...
function renderPage(data) {
  ...
  renderTheexonet(data.theexonet, data.theexonetError, data.apiPublicUrl, data.gameUrl);
}
...
async function refresh() {
  ...
  try {
    ...
    els.theexonetApiError.textContent = error.message;
  }
}
